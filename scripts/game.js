...
for (let cellIndex = 0; cellIndex < cells.length; cellIndex++) {
    ...
    cell.onclick = () => {
        ...
        let pieceElem = piece ? document.querySelector('.piece[data-index="' + piece.index + '"]') : null
        ...
        if (active != null && !piece && cell.classList.contains('possible')) {
            ...
            let pieceElem = document.querySelector('.piece[data-index="' + active + '"]')
            ...
            pieceElem.style.transform = 'translate(' + (col * 100) + '%, ' + (row * 100) + '%)'
                ...
                for (let i = 0; i < routeList.length; i++) {
                    ...
                    if (row == route.position[0] && col == route.position[1]) {
                        ...
                        for (let [r, c] of route.capture) {
                            let capturedPiece = document.querySelector('.piece[data-index="' + pieceIndexByPos(r, c) + '"]')
...
function makeQueen(piece) {
    if (pieceMap[piece.index].queen) { return }

    pieceMap[piece.index].queen = true

    let pieceElem = document.querySelector('.piece[data-index="' + piece.index + '"]')

    let queen = document.createElement('img')
    queen.src = '../png/queen_' + (piece.color == 'white' ? 'black' : 'white') + '.png'
...
function showMoves(piece) {
    ...
    function showCommonMoves() {
        for (let i = 0; i < vectors.length; i++) {
            ...
            if ((r < 0 || r > 7 || c < 0 || c > 7) ||
                (((turn == 'white' && r > row) || (turn == 'black' && r < row)) && !piece.queen) ||
                cells[r * 8 + c].dataset.piece
            ) {
                continue
            }
    ...
    function showCaptureMoves(r, c) {
        ...
        function checkCaptureAround(row, col, exclude, routeG) {
            for (let i = 0; i < vectors.length; i++) {
                let route = routeG ? JSON.parse(JSON.stringify(routeG)) : { position: 0, capture: [], queen: isQueen }
                ...
                if (cells[r1 * 8 + c1].dataset.piece &&
...
function checkWin() {
    ...
    if (stillWhite && !stillBlack) { return 'white' }
    else if (stillBlack && !stillWhite) { return 'black' }
}
...
function drawPieces() {
    for (let piece of pieceMap) {
        ...
        pieceElem.classList.add('piece', 'block', 'piece_' + piece.color, 'set_' + setNum)
